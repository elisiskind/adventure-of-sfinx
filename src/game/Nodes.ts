...
export const gameGraph = (context: CloudStorage): TextNodeGraph => {
  return {
    ...
    LEFT_1: {
      prompt: [`You look left. You see the inside of a spaceport airlock. Nothing is out of place.`,
        ...
        before the primary sun starts to rise.${context.airlockTime > 2 ? ` If you spend too long admiring the view, you might run our of air in the airlock.` : ''}`
      ],
      increaseAirlockTime: true,
      options: context.airlockTime > 4 ?
          {
    ...
    RIGHT_1: {
      prompt: `You look to the right. You see the door you just came out of.${context.airlockTime > 2 ? ` Now that it's shut, you only have about a 
    ...
    STAY_IN_FRONT_OF_DOOR: {
      prompt: `The door opens on a dark room. You can see the faint shape of a humanoid moving toward you in the ` +
    ...
    HIDE_NEXT_TO_DOOR: {
      prompt: `You quickly press yourself against the side of the door as it opens. A figure walks out, looking directly ` +
    ...
    SNEAK_THROUGH_DOOR: {
      prompt: `You run through the open door and it shuts smoothly behind you. Lights flicker on, illuminating a short ` +
    ...
    BACK_TO_BIG_ROOM: {
      prompt: `You turn back around. The door has a button next to it marked with an "open" symbol. You press it, and ` +
    ...
    GREET_PIP: {
      ...
      options: {
        ...
        CHASE_DEAD: [`"Wait, what happened to Chase?"`, `"Gravlax? Why should I trust her? Chase said she was up to no good!"`],
      }
    },
    PIP_DEAD: {
      prompt: `"Dead! Oh no, not dead. Mind you, I'm not sure that's for the best, but no, I'm still cranking along. ` +
    ...
    WHY_TRUST_GRAVLAX: {
      prompt: `Chase was wrong, Sfinx! He didn't know everything that was going on. I promise you, if you can trust me, ` +
